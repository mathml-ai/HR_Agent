...
# Resume Scorer Node
# -------------------------------
def score_resumes_gemini_raw(resume_files, job_desc_file_like):
...
Scoring rules:
1) If resume does not meet minimum requirements, score = 0. 
2) Otherwise, total score = 100, divided as:
   - 10/100: minimum requirements
   - 20/100: experience or achievements
   - 70/100: relevance to JD
...
def extract_and_sort_responses(raw_responses):
    extracted = []
    for r in raw_responses:
        try:
            match = re.search(r"\{.*\}", r, flags=re.DOTALL)
            if match:
...
# -------------------------------
    ...
    if st.button("Score") and jd and resumes:
        raw = score_resumes_gemini_raw(resumes, jd)
        sorted_responses = extract_and_sort_responses(raw)
        st.json(sorted_responses)
